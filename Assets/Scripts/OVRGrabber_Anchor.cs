...
public class OVRGrabber_Anchor : MonoBehaviour
{
	...
	protected OVRGrabbable_Anchor m_grabbedObj = null;
    protected Vector3 m_grabbedObjectPosOff;
    protected Quaternion m_grabbedObjectRotOff;
	protected Dictionary<OVRGrabbable_Anchor, int> m_grabCandidates = new Dictionary<OVRGrabbable_Anchor, int>();
	protected bool m_operatingWithoutOVRCameraRig = true;

    protected bool m_areWeGrabbing = false;

    /// <summary>
    /// The currently grabbed object.
    /// </summary>
    public OVRGrabbable_Anchor grabbedObject
    {
        get { return m_grabbedObj; }
    }

	public void ForceRelease(OVRGrabbable_Anchor grabbable)
    {
        bool canRelease = (
            (m_grabbedObj != null) &&
    ...
    void OnTriggerEnter(Collider otherCollider)
    {
        ...
        OVRGrabbable_Anchor grabbable = otherCollider.GetComponent<OVRGrabbable_Anchor>() ?? otherCollider.GetComponentInParent<OVRGrabbable_Anchor>();
    ...
    void OnTriggerExit(Collider otherCollider)
    {
        ...
        {
            // keep the candidate around if we're grabbing,
            // enabling rapid re-grabs
            if (m_grabbedObj != null)
            {
    ...
    protected void CheckForGrabOrReleaseBinary(bool isYDown)
    {
        if (m_areWeGrabbing && isYDown)
        {
            // do nothing
        }
        else if (m_areWeGrabbing && !isYDown)
        {
            GrabEnd();
            m_areWeGrabbing = false;
        }
        else if (!m_areWeGrabbing && isYDown)
        {
            GrabBegin();
            m_areWeGrabbing = true;
        }
        else
        {
            // do nothing
    ...
    protected virtual void GrabBegin()
    {
        ...
        OVRGrabbable_Anchor closestGrabbable = null;
        Collider closestGrabbableCollider = null;

        // Iterate grab candidates and find the closest grabbable candidate
		foreach (OVRGrabbable_Anchor grabbable in m_grabCandidates.Keys)
        {
            bool canGrab = !(grabbable.isGrabbed && !grabbable.allowOffhandGrab);
            //bool canGrab = grabbable.allowOffhandGrab;
            if (!canGrab)
            {
                continue;
            }
	...
	protected virtual void OffhandGrabbed(OVRGrabbable_Anchor grabbable)
    {
